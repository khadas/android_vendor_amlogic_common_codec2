...
//#define LOG_NDEBUG 0
#define LOG_TAG "C2AudioAacDecoder"
#include <log/log.h>
...
constexpr char COMPONENT_NAME[] = "c2.amlogic.audio.decoder.aac";
constexpr size_t kDefaultOutputPortDelay = 2;
constexpr size_t kMaxOutputPortDelay = 16;

class C2AudioAacDecoder::IntfImpl : public AudioDecInterface<void>::BaseParams {
public:
...
bool C2AudioAacDecoder::outputDelayRingBufferPutSamples(INT_PCM *samples, int32_t numSamples) {
    if (numSamples == 0) {
        return true;
    }
...
int32_t C2AudioAacDecoder::outputDelayRingBufferSamplesAvailable() {
    return mOutputDelayRingBufferFilled;
}

int32_t C2AudioAacDecoder::outputDelayRingBufferSpaceLeft() {
    return mOutputDelayRingBufferSize - outputDelayRingBufferSamplesAvailable();
}

void C2AudioAacDecoder::drainRingBuffer(
...
void C2AudioAacDecoder::process(
        const std::unique_ptr<C2Work> &work,
        const std::shared_ptr<C2BlockPool> &pool) {
...
c2_status_t C2AudioAacDecoder::drainInternal(
        uint32_t drainMode,
        const std::shared_ptr<C2BlockPool> &pool,
        const std::unique_ptr<C2Work> &work) {
...
c2_status_t C2AudioAacDecoder::drain(
        uint32_t drainMode,
        const std::shared_ptr<C2BlockPool> &pool) {
    return drainInternal(drainMode, pool, nullptr);
}

c2_status_t C2AudioAacDecoder::onFlush_sm() {
    drainDecoder();
    mBuffersInfo.clear();
...
void C2AudioAacDecoder::drainDecoder() {
...
uint32_t C2AudioAacDecoder::maskFromCount(uint32_t channelCount) {
...
public:
    ...
    virtual c2_status_t createComponent(
        ...
        *component = std::shared_ptr<C2Component>(
                new C2AudioAacDecoder(COMPONENT_NAME,
                              id,
                              std::make_shared<C2AudioAacDecoder::IntfImpl>(mHelper)));
        return C2_OK;
    }
    ...
    virtual c2_status_t createInterface(
        ...
        *interface = std::shared_ptr<C2ComponentInterface>(
                new AudioDecInterface<C2AudioAacDecoder::IntfImpl>(
                        COMPONENT_NAME, id, std::make_shared<C2AudioAacDecoder::IntfImpl>(mHelper)));
        return C2_OK;
    }
...
extern "C" ::C2ComponentFactory* CreateC2AudioDecoderAACFactory() {
    ALOGV("in %s", __func__);
    return new ::android::C2SoftAacDecFactory();
}

__attribute__((cfi_canonical_jump_table))
extern "C" void DestroyC2AudioDecoderAACFactory(::C2ComponentFactory* factory) {
    ALOGV("in %s", __func__);
    delete factory;
}
