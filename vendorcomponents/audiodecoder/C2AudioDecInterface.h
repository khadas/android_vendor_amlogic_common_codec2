...
#ifndef ANDROID_C2_AUDIO_DEC_INTERFACE_H_
#define ANDROID_C2_AUDIO_DEC_INTERFACE_H_
 ...
 */
template <typename T>
class C2AudioDecInterface : public C2ComponentInterface {
public:
    C2AudioDecInterface(const char *name, c2_node_id_t id, const std::shared_ptr<T> &impl)
        : mName(name),
          mId(id),
          mImpl(impl) {
    }

    ~C2AudioDecInterface() override = default;
 ...
 */
template<>
class C2AudioDecInterface<void> {
public:
    /**
...
template<typename T>
using AudioDecInterface = C2AudioDecInterface<T>;
...
#endif  // ANDROID_C2_AUDIO_DEC_INTERFACE_H_
