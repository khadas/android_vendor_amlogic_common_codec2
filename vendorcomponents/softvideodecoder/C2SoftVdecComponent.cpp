...
#define LOG_NDEBUG 0
#define LOG_TAG "C2SoftVdecComponent"
...
std::unique_ptr<C2Work> C2SoftVdecComponent::WorkQueue::pop_front() {
    std::unique_ptr<C2Work> work = std::move(mQueue.front().work);
    mQueue.pop_front();
    return work;
}

void C2SoftVdecComponent::WorkQueue::push_back(std::unique_ptr<C2Work> work) {
    mQueue.push_back({ std::move(work), NO_DRAIN });
}

bool C2SoftVdecComponent::WorkQueue::empty() const {
    return mQueue.empty();
}

void C2SoftVdecComponent::WorkQueue::clear() {
    mQueue.clear();
}

uint32_t C2SoftVdecComponent::WorkQueue::drainMode() const {
    return mQueue.front().drainMode;
}

void C2SoftVdecComponent::WorkQueue::markDrain(uint32_t drainMode) {
    mQueue.push_back({ nullptr, drainMode });
}

////////////////////////////////////////////////////////////////////////////////

C2SoftVdecComponent::WorkHandler::WorkHandler() : mRunning(false) {}

void C2SoftVdecComponent::WorkHandler::setComponent(
        const std::shared_ptr<C2SoftVdecComponent> &thiz) {
    mThiz = thiz;
}
...
void C2SoftVdecComponent::WorkHandler::onMessageReceived(const sp<AMessage> &msg) {
    std::shared_ptr<C2SoftVdecComponent> thiz = mThiz.lock();
    if (!thiz) {
        CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Component not yet set; msg = %s", msg->debugString().c_str());
        sp<AReplyToken> replyId;
    ...
    switch (msg->what()) {
        ...
        default: {
            CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Unrecognized msg: %d", msg->what());
            break;
        }
    }
}

class C2SoftVdecComponent::BlockingBlockPool : public C2BlockPool {
public:
...
c2_status_t C2SoftVdecComponent::setListener_vb(
...
c2_status_t C2SoftVdecComponent::queue_nb(std::list<std::unique_ptr<C2Work>> * const items) {
    CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "%s", __func__);
    {
        Mutexed<ExecState>::Locked state(mExecState);
        if (state->mState != RUNNING) {
...
c2_status_t C2SoftVdecComponent::announce_nb(const std::vector<C2WorkOutline> &items) {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    (void)items;
    return C2_OMITTED;
}

c2_status_t C2SoftVdecComponent::flush_sm(
...
c2_status_t C2SoftVdecComponent::drain_nb(drain_mode_t drainMode) {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
...
c2_status_t C2SoftVdecComponent::start() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    Mutexed<ExecState>::Locked state(mExecState);
    if (state->mState == RUNNING) {
        return C2_BAD_STATE;
...
c2_status_t C2SoftVdecComponent::stop() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    {
        Mutexed<ExecState>::Locked state(mExecState);
        if (state->mState != RUNNING) {
...
c2_status_t C2SoftVdecComponent::reset() {
    CODEC2_LOG(CODEC2_LOG_INFO, "%s", __func__);
    {
        Mutexed<ExecState>::Locked state(mExecState);
...
std::shared_ptr<C2ComponentInterface> C2SoftVdecComponent::intf() {
    return mIntf;
}
...
void C2SoftVdecComponent::cloneAndSend(
        uint64_t frameIndex,
...
bool C2SoftVdecComponent::processQueue() {
    ...
    CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Start processing frame #%" PRIu64, work->input.ordinal.frameIndex.peeku());
    ...
    if (!work->input.buffers.empty() && !work->input.buffers[0]) {
        CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Encountered null input buffer. Clearing the input buffer");
        work->input.buffers.clear();
    }
    process(work, mOutputBlockPool);
    CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Processed frame #%" PRIu64, work->input.ordinal.frameIndex.peeku());
    Mutexed<WorkQueue>::Locked queue(mWorkQueue);
    if (queue->generation() != generation) {
        CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Work form old generation: was %" PRIu64 " now %" PRIu64,
    ...
    if (work->workletsProcessed != 0u) {
        queue.unlock();
        Mutexed<ExecState>::Locked state(mExecState);
        CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Returning this work");
        std::shared_ptr<C2Component::Listener> listener = state->mListener;
        state.unlock();
        listener->onWorkDone_nb(shared_from_this(), vec(work));
    } else {
        CODEC2_LOG(CODEC2_LOG_DEBUG_LEVEL1, "Queue pending work");
        work->input.buffers.clear();
...
int C2SoftVdecComponent::getHalPixelFormatForBitDepth10(bool allowRGBA1010102) {
...
std::shared_ptr<C2Buffer> C2SoftVdecComponent::createLinearBuffer(
        const std::shared_ptr<C2LinearBlock> &block, size_t offset, size_t size) {
    return C2Buffer::CreateLinearBuffer(block->share(offset, size, ::C2Fence()));
}

std::shared_ptr<C2Buffer> C2SoftVdecComponent::createGraphicBuffer(
